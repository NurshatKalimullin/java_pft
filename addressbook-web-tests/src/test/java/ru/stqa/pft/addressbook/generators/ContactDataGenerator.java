    ...
    private void saveAsCsv(List<ContactData> contacts, File file) throws IOException {
        ...
        for (ContactData contact : contacts){
            writer.write(String.format("%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s\n", contact.getFirstName(),
                    contact.getLastName(), contact.getNickname(), contact.getCompanyType(),
                    contact.getCompanyName(), contact.getHomeAddress(), contact.getHomePhone(),
                    contact.getEmail(), contact.getEmail2(), contact.getEmail3(), contact.getGroup(),
                    contact.getMobilePhone(), contact.getWorkPhone(), contact.getPhoto()));
        }
        writer.close();
    }
